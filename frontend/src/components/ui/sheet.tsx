...
function SheetContent({
  ...
  // On mobile, force bottom side for right/left sheets
  const effectiveSide = isMobile && (side === "right" || side === "left") ? "bottom" : side;
...
function SheetHeader({ className, children, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("sticky top-0 z-10 flex flex-col gap-1.5 p-4 pb-3 bg-background border-b", className)}
      {...props}
    >
      {children}
