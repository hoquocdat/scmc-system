...
interface ServiceOrderImagesTabProps {
  serviceOrderId: string;
  orderNumber: string;
  onUpdate?: () => void;
}

export function ServiceOrderImagesTab({ serviceOrderId, orderNumber, onUpdate }: ServiceOrderImagesTabProps) {
  const [uploadingFiles, setUploadingFiles] = useState<UploadFile[]>([]);
  const [isSelectMode, setIsSelectMode] = useState(false);
  const [selectedImages, setSelectedImages] = useState<Set<string>>(new Set());

  // Fetch images using React Query
  const { data: imageRecords } = useEntityImages('service_order', serviceOrderId);

  // Upload mutation
  const uploadMutation = useUploadImages('service_order', serviceOrderId);

  // Delete mutation
  const deleteMutation = useDeleteImage('service_order', serviceOrderId);
    ...
    <div className="space-y-6">
      {images?.length === 0 ? (
        ...
        <>
          ...
          <PhotoGallery
            images={images}
            altPrefix={`Service Order ${orderNumber}`}
