...
export class SalesService {
  ...
  async create(createSalesOrderDto: CreateSalesOrderDto) {
    try {
      ...
      const pointsToRedeem = orderData.points_to_redeem || 0;

      if (pointsToRedeem > 0 && orderData.customer_id) {
        // Validate and calculate loyalty discount
        const preliminaryTotal = subtotal - discountAmount + taxAmount + (orderData.shipping_cost || 0);
        const redemptionPreview = await this.loyaltyService.calculateRedemption({
        ...
        if (pointsToRedeem < redemptionPreview.minRedemptionPoints) {
          throw new BadRequestException(
            `Minimum redemption is ${redemptionPreview.minRedemptionPoints} points`,
          );
        }

        if (pointsToRedeem > redemptionPreview.maxRedeemablePoints) {
          throw new BadRequestException(
            `Maximum redeemable points is ${redemptionPreview.maxRedeemablePoints}`,
          );
        }

        loyaltyDiscountAmount = pointsToRedeem * redemptionPreview.redemptionRate;
      }

      const totalAmount =
        orderData.total_amount ||
        subtotal - discountAmount - loyaltyDiscountAmount + taxAmount + (orderData.shipping_cost || 0);
      ...
      const data: Prisma.sales_ordersCreateInput = {
        ...
        loyalty_discount_amount: new Prisma.Decimal(loyaltyDiscountAmount),
      ...
      // Process loyalty point redemption after order is created
      if (pointsToRedeem > 0 && orderData.customer_id) {
        try {
          ...
          // Log but don't fail order creation if redemption fails
  ...
  async cancel(id: string) {
    try {
      ...
      // Reverse loyalty points if customer exists and order had points transactions
      if (order.customer_id) {
        try {
          await this.loyaltyService.reversePoints(
            order.customer_id,
            order.id,
            'sales_order',
          );
          ...
          console.error('Failed to reverse loyalty points:', loyaltyError);
        }
      }
  ...
  async addPayment(createPaymentDto: CreatePaymentDto) {
    try {
      ...
      if (
        ...
        try {
          // Calculate eligible amount (total minus loyalty discount already applied)
          const loyaltyDiscount = Number(order.loyalty_discount_amount || 0);
          const eligibleAmount = totalAmount - loyaltyDiscount;
          ...
          const earnResult = await this.loyaltyService.earnPoints({
            ...
            createdBy: createPaymentDto.received_by,
          });

          // Update order with earned points
          if (earnResult.pointsEarned > 0) {
            await this.prisma.sales_orders.update({
              where: { id: order.id },
              data: {
                loyalty_points_earned: earnResult.pointsEarned,
              },
            });
          }
        } catch (loyaltyError) {
          // Log but don't fail the payment if loyalty fails
