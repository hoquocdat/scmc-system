...
async function seedBrandsAndModels() {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      console.log(`   - Total brands in database: ${brandsData.length}`);
      console.log(`   - Total models in database: ${brandsData.reduce((sum, b) => sum + b.models.length, 0)}`);
    });
  } catch (error) {
    console.error('❌ Error seeding brands and models:', error);
    throw error;
  }
}
