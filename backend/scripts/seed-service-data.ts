...
// Service order templates (will calculate dates dynamically)
...
async function seedServiceData() {
  ...
  // Step 2: Insert Bikes
  console.log('\n🏍️  Creating bikes...');
  const bikesToInsert = bikesData.map((bike) => ({
  ...
  const { data: createdBikes, error: bikesError } = await supabase
    .from('bikes')
  ...
  if (techError || !technicians || technicians.length === 0) {
    console.error('❌ No technicians found. Please run seed-employees.ts first.');
    return;
  }
  ...
  const serviceOrdersToInsert = serviceOrdersData.map((order) => {
  ...
  const { data: bikeCount } = await supabase
    .from('bikes')
    .select('id', { count: 'exact', head: true });
  ...
  const { data: inServiceOrders } = await supabase
    .from('service_orders')
    .select('id')
...
seedServiceData().catch(console.error);
